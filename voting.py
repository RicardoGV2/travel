...
def update_votes(selected_date, selected_time, selected_option):
    current_vote = st.session_state['user_votes'][st.session_state['user']][selected_date][selected_time]
    if current_vote:
        votes[selected_date][selected_time][current_vote] -= 1
    votes[selected_date][selected_time][selected_option] += 1
    st.session_state['user_votes'][st.session_state['user']][selected_date][selected_time] = selected_option
...
for time in sorted(data[selected_date]):
    ...
    if st.button(f"Vote for {selected_option}", key=f"button_{selected_date}_{time}"):
        current_vote = st.session_state['user_votes'][st.session_state['user']][selected_date][time]
        if current_vote:
            vote_counts[current_vote] -= 1
        vote_counts[selected_option] += 1
        st.session_state['user_votes'][st.session_state['user']][selected_date][time] = selected_option
